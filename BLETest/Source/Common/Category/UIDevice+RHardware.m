...
@implementation UIDevice (RHardware)
/*
 Platforms
...
{
    NSString *platform = [self tbcPlatform];
...
+ (double)loadAutoLayoutRate
{
    ...
    if (rate == -1) {
        rate = [[UIScreen mainScreen] bounds].size.width/320.0f;
    }
    return rate;
}
...
{
    NSUInteger type = [self tbcPlatformType];
...
- (BOOL)isIOS6 {
    ...
    if (result == -1) {
        ...
        result = majorVersion.integerValue >= 6;
    }
    return (BOOL)result;
}
...
{
    BOOL result = NO;
    BOOL isIphone6OrPlus = [[self tbcPlatformString] isEqualToString:IPHONE_6_NAMESTRING] || [[self tbcPlatformString] isEqualToString:IPHONE_6_PLUS_NAMESTRING];
    if ([[UIDevice currentDevice] tbcPlatformType] == UIDeviceSimulatoriPhone) {
        isIphone6OrPlus = ([UIScreen mainScreen].bounds.size.width > 320);
    }
    if ([self isIOS8] && isIphone6OrPlus)
    {
        result = YES;
    }
    return result;
...
{
    BOOL result = NO;
    BOOL isIphone6Plus = [UIScreen mainScreen].scale == 3 && [[self tbcPlatformString] isEqualToString:IPHONE_6_PLUS_NAMESTRING];
    if ([[UIDevice currentDevice] tbcPlatformType] == UIDeviceSimulatoriPhone) {
        isIphone6Plus = ([UIScreen mainScreen].scale == 3);
    }
    if ([self isIOS8] && isIphone6Plus) {
        result = YES;
    }
    return result;
}

// 只是iphone6plus正常模式（非放大模式）
- (BOOL)isIphone6PlusNormal
{
    return ([self isIphone6Plus] && ![self isIphone6PlusBig]);
}

- (BOOL)isIphone6PlusBig
{
    BOOL isBig = ([[UIDevice currentDevice] currentDeviceResolutionWidth] == 375);
    if ([self isIphone6Plus] && isBig) {
        return YES;
    }
    return NO;
}
...
{
    BOOL result = NO;
    BOOL isIphone6 = [UIScreen mainScreen].scale == 2 && [[self tbcPlatformString] isEqualToString:IPHONE_6_NAMESTRING];
    if ([[UIDevice currentDevice] tbcPlatformType] == UIDeviceSimulatoriPhone) {
        isIphone6 = ([[UIDevice currentDevice] currentDeviceResolutionWidth] == 375);
    }
    if ([self isIOS8] && isIphone6) {
        result = YES;
    }
    return result;
}
// 只是iphone6的正常模式（非放大模式）
- (BOOL)isIphone6Normal
{
    return ([self isIphone6] && ![self isIphone6Big]);
}

- (BOOL)isIphone6Big
{
    BOOL isBig = ([[UIDevice currentDevice] currentDeviceResolutionWidth] == 320);
    if ([self isIphone6] && isBig) {
        return YES;
    }
    return NO;
}
