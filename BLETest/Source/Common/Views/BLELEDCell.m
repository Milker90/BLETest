...
@interface BLELEDCell()

@property (nonatomic, weak) BLELEDItem *item;
...
- (void)updateUI:(id)data {
    if (!data || ![data isKindOfClass:[BLELEDItem class]]) {
        return;
    }
    
    BLELEDItem *item = (BLELEDItem *)data;
    self.item = item;
...
- (void)bleAction:(UIButton *)button {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(ledAction:)]) {
        [_delegate ledAction:_item];
    }
}
